...
public class BaseBuildings {
	...
	public void buildBase(Panel field, Panel console, Loader load, Funktions func, Game game, String FirstColor, String SecColor, int default_position_X, int default_position_Y){
			...
			public void mouseReleased(MouseEvent objUnit) {
				for (int i = 0; i < BuildingsEntity.size(); i++) {
					if (BuildingsEntity.get(i).getLabel() == objUnit.getSource()){
						game.createAktion(BuildingsEntity, i);
