...
public class Loader {
	...
	public void switchPanel(String newPage) {
		if (newPage == Mainpage)
			...
			window.setContentPane(new Panel(Lobbypage, HEIGHT, WIGTH, this));
		else if (newPage == Gamepage)
			window.setContentPane(new Panel(Gamepage, HEIGHT, WIGTH, this));
		else if (newPage == Grouppage)
			window.setContentPane(new Panel(Grouppage, HEIGHT, WIGTH, this));
		else if (newPage == Grouppage_owner)
			window.setContentPane(new Panel(Grouppage_owner, HEIGHT, WIGTH,
					this));
		window.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		window.setResizable(false);
		window.pack();
